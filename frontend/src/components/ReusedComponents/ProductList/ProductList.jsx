...
const ProductList = ({ productList, title,  }) => {
  ...
  const breadcrumbs = location.pathname.split('/').filter((path) => path).map((path, index, arr) => {
          ...
          <Grid item xs={12} sm={6} md={4} key={index}>
            <Card className='product-card-section'>
              ...
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary" paragraph style={{color: "white"}}>
                  {product.description}
                </Typography>
